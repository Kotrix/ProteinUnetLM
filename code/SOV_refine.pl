...
print "Lambda\t$lambda\n";
for (my $i = 0; $i < @states; $i++) {
  my $tmp_sovi = sprintf "%0.4f", $SOVs_99[$i];
  print "SOV_99_i\t$states[$i]\t$tmp_sovi\n";
	$tmp_sovi = sprintf "%0.4f", $SOVs_refine[$i];
	print "SOV_refine_i\t$states[$i]\t$tmp_sovi\n";
}

$accuracy = sprintf "%0.4f", $accuracy;
$SOV_all_99 = sprintf "%0.4f", $SOV_all_99;
$SOV_all_refine = sprintf "%0.4f", $SOV_all_refine;
